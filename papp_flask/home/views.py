...
def index():
    if request.method == 'POST':
        ethereum_address = request.form.get('ethereum_address', None)
        tokenid = request.form.get('tokenid', None)
        if ethereum_address:
            ...
            flash("No image found for the provided Ethereum address")
            return render_template('index.html')
        # User provided a tokenId
        if tokenid:
            # print(tokenid)
            url = f"https://base-mainnet.g.alchemy.com/nft/v3/{ALCHEMY_API_KEY}/getNFTMetadata?contractAddress={CONTRACT_ADDRESS}&tokenId={tokenid}&refreshCache=false"
            headers = {"accept": "application/json"}
            response = requests.get(url, headers=headers)
            data = response.json()
            nfts = []
            if data['name'] is None:
                # TokenID not found AKA 100001+
                flash("No tokenID found")
                return render_template('index.html')

            if 'name' in data and 'tokenId' in data and 'image' in data and 'originalUrl' in data['image']:
                name = data['name']
                token_id = data['tokenId']
                description = data['description']
                svg_base64 = data['image']['originalUrl'].split(',')[1]
                svg_data = base64.b64decode(svg_base64)
                upscaled_image = upscale_svg(svg_data, 1024, 1024)
                # Convert PIL image to bytes
                png_bytes = io.BytesIO()
